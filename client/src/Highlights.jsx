import React from 'react';

const Highlights = () => (
  <div>
  <div>
    <div>
      <div style="margin-bottom:24px">
      <div class="_1d6n3ar">
        <small class="_1wk8be92">
          <span>HOME HIGHLIGHTS</span>
        </small>
      <div style="margin-top:12px">
        <div>
          <div style="margin-bottom:8px">
            <span class="_okqgdgp">Hot tub<span aria-hidden="true"> · </span></span>
            <span class="_jz15f0c">This is one of few homes in this area that has this feature.</span>
          </div>
        <div class="_5y40cyp">
      <div class="_10k87om">
    <div>
      <button type="button" class="_aijsh5" aria-busy="false">
        <div class="_qtix31">
          <div class="_ni9axhe">
            <span>Helpful</span>
          </div>
            <div class="_ni9axhe">
              <div style="margin-left:4px">
                <svg viewBox="0 0 16 16" role="presentation" aria-hidden="true" focusable="false" style="height:1em;width:1em;display:block;fill:currentColor">
                  <path d="m8.37 1c-.34 0-.53.43-.64.79l-.04.17c-.17.74-.56 2.47-3.76 3.58-1.97.68-2.93 2.31-2.93 4.96 0 1.68.56 4.5 4.3 4.5h6.2c.3 0 1-.06 1-.63 0-.41-.26-.63-.5-.63a.5.5 0 1 1 0-1c .92 0 1-.31 1-.63 0-.52-.38-.61-.54-.63a.5.5 0 0 1 -.46-.52.5.5 0 0 1 .5-.48c1 0 1-.41 1-.63s0-.62-1-.62a.5.5 0 1 1 0-1c1 0 1-.47 1-.63 0-.58-.77-.63-1-.63h-4.5a1 1 0 0 1 -.83-1.56c.05-.07.16-.19.31-.35.34-.35.92-.93 1.21-1.58.24-.52.35-1.25.28-1.78 0-.01-.12-.74-.62-.74m3.15 15.04h-6.2c-3.32 0-5.3-2.06-5.3-5.5 0-3.09 1.21-5.08 3.61-5.91 2.67-.93 2.97-2.23 3.11-2.86.02-.08.04-.16.05-.22.37-1.31 1.16-1.51 1.6-1.51 1.02 0 1.52.96 1.61 1.61.09.71-.06 1.64-.37 2.32-.37.81-1.02 1.47-1.41 1.86-.09.1-.17.17-.2.21h4.5c1.2 0 2 .65 2 1.63 0 .46-.16.84-.45 1.11.39.37.45.86.45 1.14 0 .34-.09.98-.74 1.35.15.23.24.53.24.9 0 .41-.12 1.01-.74 1.36.15.26.24.56.24.89 0 .97-.8 1.63-2 1.63">
                  </path>
                </svg>
              </div>
            </div>
          </div>
        </button>
          <div class="_36rlri" style="margin-left:4px;margin-right:4px">
            <span aria-hidden="true"> · </span>
          </div>
            <button type="button" class="_aijsh5" aria-busy="false">
              <span>Not helpful</span>
            </button>
          </div>
        </div>
      </div>
    </div>
  </div>
    <div style="margin-top:12px">
      <div>
        <div style="margin-bottom:8px">
          <span class="_okqgdgp">Laskarina is a Superhost<span aria-hidden="true"> · </span></span>
          <span class="_jz15f0c">Superhosts are experienced, highly rated hosts who are committed to providing great stays for guests.</span>
        </div>
          <div class="_5y40cyp">
            <div class="_10k87om">
              <div>
                <button type="button" class="_aijsh5" aria-busy="false">
                  <div class="_qtix31">
                    <div class="_ni9axhe">
                      <span>Helpful</span>
                    </div>
                      <div class="_ni9axhe">
                        <div style="margin-left:4px">
                          <svg viewBox="0 0 16 16" role="presentation" aria-hidden="true" focusable="false" style="height:1em;width:1em;display:block;fill:currentColor">
                          <path d="m8.37 1c-.34 0-.53.43-.64.79l-.04.17c-.17.74-.56 2.47-3.76 3.58-1.97.68-2.93 2.31-2.93 4.96 0 1.68.56 4.5 4.3 4.5h6.2c.3 0 1-.06 1-.63 0-.41-.26-.63-.5-.63a.5.5 0 1 1 0-1c .92 0 1-.31 1-.63 0-.52-.38-.61-.54-.63a.5.5 0 0 1 -.46-.52.5.5 0 0 1 .5-.48c1 0 1-.41 1-.63s0-.62-1-.62a.5.5 0 1 1 0-1c1 0 1-.47 1-.63 0-.58-.77-.63-1-.63h-4.5a1 1 0 0 1 -.83-1.56c.05-.07.16-.19.31-.35.34-.35.92-.93 1.21-1.58.24-.52.35-1.25.28-1.78 0-.01-.12-.74-.62-.74m3.15 15.04h-6.2c-3.32 0-5.3-2.06-5.3-5.5 0-3.09 1.21-5.08 3.61-5.91 2.67-.93 2.97-2.23 3.11-2.86.02-.08.04-.16.05-.22.37-1.31 1.16-1.51 1.6-1.51 1.02 0 1.52.96 1.61 1.61.09.71-.06 1.64-.37 2.32-.37.81-1.02 1.47-1.41 1.86-.09.1-.17.17-.2.21h4.5c1.2 0 2 .65 2 1.63 0 .46-.16.84-.45 1.11.39.37.45.86.45 1.14 0 .34-.09.98-.74 1.35.15.23.24.53.24.9 0 .41-.12 1.01-.74 1.36.15.26.24.56.24.89 0 .97-.8 1.63-2 1.63"></path>
                          </svg>
                        </div>
                      </div>
                    </div>
                </button>
                  <div class="_36rlri" style="margin-left:4px;margin-right:4px">
                    <span aria-hidden="true"> · </span>
                  </div>
                    <button type="button" class="_aijsh5" aria-busy="false">
                      <span>Not helpful</span>
                    </button>
                  </div>
                </div>
              </div>
            </div>
          </div>
            <div style="margin-top:12px">
              <div>
                <div style="margin-bottom:8px">
                  <span class="_okqgdgp">Great check-in experience<span aria-hidden="true"> · </span></span>
                  <span class="_jz15f0c">100% of recent guests gave this home’s check-in process a 5-star rating.</span>
                </div>
                  <div class="_5y40cyp">
                    <div class="_10k87om">
                      <div>
                        <button type="button" class="_aijsh5" aria-busy="false">
                          <div class="_qtix31">
                            <div class="_ni9axhe">
                              <span>Helpful</span>
                            </div>
                              <div class="_ni9axhe">
                                <div style="margin-left:4px">
                                  <svg viewBox="0 0 16 16" role="presentation" aria-hidden="true" focusable="false" style="height:1em;width:1em;display:block;fill:currentColor">
                                  <path d="m8.37 1c-.34 0-.53.43-.64.79l-.04.17c-.17.74-.56 2.47-3.76 3.58-1.97.68-2.93 2.31-2.93 4.96 0 1.68.56 4.5 4.3 4.5h6.2c.3 0 1-.06 1-.63 0-.41-.26-.63-.5-.63a.5.5 0 1 1 0-1c .92 0 1-.31 1-.63 0-.52-.38-.61-.54-.63a.5.5 0 0 1 -.46-.52.5.5 0 0 1 .5-.48c1 0 1-.41 1-.63s0-.62-1-.62a.5.5 0 1 1 0-1c1 0 1-.47 1-.63 0-.58-.77-.63-1-.63h-4.5a1 1 0 0 1 -.83-1.56c.05-.07.16-.19.31-.35.34-.35.92-.93 1.21-1.58.24-.52.35-1.25.28-1.78 0-.01-.12-.74-.62-.74m3.15 15.04h-6.2c-3.32 0-5.3-2.06-5.3-5.5 0-3.09 1.21-5.08 3.61-5.91 2.67-.93 2.97-2.23 3.11-2.86.02-.08.04-.16.05-.22.37-1.31 1.16-1.51 1.6-1.51 1.02 0 1.52.96 1.61 1.61.09.71-.06 1.64-.37 2.32-.37.81-1.02 1.47-1.41 1.86-.09.1-.17.17-.2.21h4.5c1.2 0 2 .65 2 1.63 0 .46-.16.84-.45 1.11.39.37.45.86.45 1.14 0 .34-.09.98-.74 1.35.15.23.24.53.24.9 0 .41-.12 1.01-.74 1.36.15.26.24.56.24.89 0 .97-.8 1.63-2 1.63"></path>
                                  </svg>
                                </div>
                              </div>
                            </div>
                          </button>
                            <div class="_36rlri" style="margin-left:4px;margin-right:4px">
                              <span aria-hidden="true"> · </span>
                            </div>
                              <button type="button" class="_aijsh5" aria-busy="false">
                                <span>Not helpful</span>
                              </button>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
);

export default Highlights;
